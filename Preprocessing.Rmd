...
levels(silva_taxonomy_16s_clean$High_level_taxonomy)
...
```{r}
physeq_prok_nonorm_filtax <- ReformatTaxonomy(physeq_prok_nonfilt)
head(tax_table(physeq_prok_nonorm_filtax))
saveRDS(physeq_prok_nonorm_filtax, "physeq_prok_nonorm_filtax_010621.rds")
```

## DECONTAMINATE
-Use the full dataset to call contaminants, then remove them, if they exist in the non plant OTU dataset
```{r Bacteria}
physeq_prok_nonorm_filtax@sam_data$Sample_or_Control <- ifelse(physeq_prok_nonorm_filtax@sam_data$Objective == "NTC", "Control Sample", "True Sample")
sample_data(physeq_prok_nonorm_filtax)$is.neg <- sample_data(physeq_prok_nonorm_filtax)$Sample_or_Control == "Control Sample"
contamdf.prev <- isContaminant(physeq_prok_nonorm_filtax, method="prevalence", neg="is.neg", threshold = 0.1, normalize = TRUE)
badTaxa <- rownames(contamdf.prev[contamdf.prev$contaminant == TRUE,])

print(badTaxa)

ps.pa <- transform_sample_counts(physeq_prok_nonorm_filtax, function(abund) 1*(abund>0))
...
goodTaxa <- setdiff(taxa_names(physeq_prok_nonorm_filtax), badTaxa)
bacteria_sub_no_bad <- prune_taxa(goodTaxa, physeq_prok_nonorm_filtax)
```
...
saveRDS(bacteria.no.plant.obj2, file = "Fungicide_Bacteria_RootsAndLeaves_obj2_unedited.rds")
# Restore the object
bacteria.no.plant.obj2.unedited <- readRDS(file = "Fungicide_Bacteria_RootsAndLeaves_obj2_unedited.rds")
```
...
```
Final total for bacteria - 29,729,595 reads across 555 samples 

```{r FUNGI X diff.}
mean(sample_sums(bacteria.no.plant.obj2_1000reads)) # 53566.84
median(sample_sums(bacteria.no.plant.obj2_1000reads)) # 47542
```
 
## BACTERIA NO NORM
```{r BACTERIA No Norm - RDS}
# Save an object to a file
saveRDS(bacteria.no.plant.obj2_1000reads, file = "Fungicide_Bacteria_RootsAndLeaves_newtaxa_nonorm_010621.rds")
# Restore the object
bacteria.no.norm <- readRDS(file = "Fungicide_Bacteria_RootsAndLeaves_newtaxa_nonorm_010621.rds")
```
...
# Save an object to a file
saveRDS(bacteria.css.norm, file = "Fungicide_Bacteria_RootsAndLeaves_newtaxa_CSSnorm_010621.rds")
# Restore the object
bacteria.css.norm <- readRDS(file = "Fungicide_Bacteria_RootsAndLeaves_newtaxa_CSSnorm_010621.rds")
```
...
```{r}
fungi_sub_no_bad %>% 
  subset_samples(Objective == "PC") %>%
  phyloseq::filter_taxa(function(x) sum(x) > 10, TRUE) %>%
  plot_bar(fill="Species")# remove taxa with zero reads (i.e., those not present in objective 2)
```
...
```{r FUNGI UNEDITED}
# Save an object to a file
saveRDS(fungi_obj2, file = "Fungicide_Fungi_RootsAndLeaves_obj2_unedited.rds")
# Restore the object
fungi.obj2.unedited <- readRDS(file = "Fungicide_Fungi_RootsAndLeaves_obj2_unedited.rds")
```

### FILTER FUNGI TO 1000 reads per sample
```{r}
fungi_obj2_1000reads <- prune_samples(sample_sums(fungi_obj2) > 1000, fungi_obj2) # filter samples that did not sequence at all
```
...
max(sample_sums(fungi_obj2_1000reads))/min(sample_sums(fungi_obj2_1000reads))
```
164X difference in sequence depth, when cutting at 1000 reads/sample I will use CSS normalization. Not going to fix the problem, but its better than not normalizing. According to Weiss et al. 2017 a 10X difference in library sizes is a cuttoff for rarefying. 
## Load Non-Normalized Data
```{r FUNGI No Norm - RDS}
# Save an object to a file
saveRDS(fungi_obj2_1000reads, file = "Fungicide_Fungi_RootsAndLeaves_1000readsmin_nonorm.rds")
# Restore the object
fungi.no.norm <- readRDS(file = "Fungicide_Fungi_RootsAndLeaves_1000readsmin_nonorm.rds")
```
...
# Save an object to a file
saveRDS(fungi.css.norm, file = "Fungicide_Fungi_RootsAndLeaves_1000seqMin_CSSNorm.rds")
# Restore the object
fungi.css.norm <- readRDS(file = "Fungicide_Fungi_RootsAndLeaves_1000seqMin_CSSNorm.rds")
```
