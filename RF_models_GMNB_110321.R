...
# >>> Hypothesis <<< ---------------------------------------------------------------------------------------------
# Is there some variation in fungal apha diversity data that can be explaiend by bacterial 
...
SubsetData <- function(physeq_1, physeq_2){
  ...
  count(as.data.frame(as.matrix(sample_data(physeq_new))),
...
FilterFamily <- function(physeq){
  ...
  metadata$shannon <- diversity(otu,index = "shannon", MARGIN=1) 
  print(dim(metadata))
  print(dim(otu))
...
count(physeq_fungi_CSS_Bul[[2]],Crop,DateSampled,Treatment)

FilterFamily(physeq_fungi_no_norm_sub) -> physeq_fungi_no_norm_Bul
physeq_fungi_no_norm_Bul[1]
physeq_fungi_no_norm_Bul[2]
count(physeq_fungi_no_norm_sub[[2]],Crop,DateSampled,Treatment)
...
identical(rownames(as.data.frame(physeq_fungi_no_norm_Bul[2])), rownames(otu_prok_tran))
sample_order <-
  match(rownames(as.data.frame(physeq_fungi_no_norm_Bul[2])), rownames(otu_prok_tran))
otu_prok_tran <- otu_prok_tran[sample_order,]
identical(rownames(as.data.frame(physeq_fungi_no_norm_Bul[2])), rownames(otu_prok_tran))
dim(otu_prok_tran)
range(otu_prok_tran)

# >>> A) Model: bacterial otu -> fungal richness -----------------------------------------------------------------
# adding the classification variable and sample names 
otu_prok_tran$Richness <-
  as.data.frame(physeq_fungi_no_norm_Bul[2])$richness
head(otu_prok_tran)
dim(otu_prok_tran)

# Recursive feature selection
set.seed(110321)
rfe_prok_rich <- Boruta(
  Richness ~ .,
  otu_prok_tran,
  pValue = 0.05,
  mcAdj = TRUE,
...
rfe_prok_rich <-
  getSelectedAttributes(rfe_prok_rich, withTentative = TRUE)
rfe_prok_rich

otu_prok_tran[, c(rfe_prok_rich)] -> otu_prok_tran_sel
...
perm_RF_prok_rich # model significant = 0.002

# *** FIGURE RF model Richness ---------------------------------------------------------------------------------
...
df_melt$Date <-
  factor(df_melt$Date, levels = c("3-Aug-18", "16-Aug-18", "27-Aug-18"))

df_melt$OTU <-
  factor(df_melt$OTU, levels = c("BOTU_10215","BOTU_24", "BOTU_1874",
                                 "BOTU_5403", "BOTU_21","BOTU_80",
                                 "BOTU_22","BOTU_6451", "BOTU_41"))
...
ggarrange(PlotError(RF_prok_rich) +
          ...
          PlotLine(RF_prok_rich,  as.data.frame(physeq_fungi_no_norm_Bul[2])) + 
            theme(legend.position = c(0.2, 0.95)) +
            labs(title = "Bulleribacidaceae\nrichness ", y="Predicted", x="Observed"),
          PlotFeature(RF_prok_rich, taxa_prok_no_norm_sub) +  
          ...
          align = "h",
          ncol = 4, 
          nrow =1) -> Fig_1_RF_rich

Fig_1_RF_rich

# >>> B) Control Model: bacterial otu -> fungal richness ------------------------------------------------------------
...
perm_RF_prok_rich_CR <- rf.significance(x=RF_prok_rich_CR, xdata=otu_prok_tran_sel_CR[,1:(ncol(otu_prok_tran_sel_CR)-1)],
                                        ...
                                        nmtry=1, 
                                        ntree=1001)  
perm_RF_prok_rich_CR # model significant = 0.001

# >>> C) Model: bacterial otu -> fungal NMDS1 --------------------------------------------------------------------
...
# *** FIGURE RF model Beta Diversity ---------------------------------------------------------------------------
...
# >>> D) Model Control: bacterial otu -> fungal NMDS1 ------------------------------------------------------------
...
perm_RF_prok_nmds1_CD # model significant = 0.001
...
# *** FIGURE 1 ---------------------------------------------------------------------------------------------------
Fig_X_RF_Bul <-
  ggarrange(PlotLine(RF_prok_rich,  as.data.frame(physeq_fungi_no_norm_Bul[2])) + 
              theme(legend.position = c(0.2, 0.95)) +
...
PlotLine <- function(rf_model, metadata){
  ...
  if (identical(rownames(metadata), rownames(df_model))==TRUE){
    ...
    ggplot(data=df_model, aes(x=actual, y=pred)) +
      ...
      geom_smooth(method = "lm", formula = "y ~ x", se = TRUE, color="red", size=0.8) +
      theme_classic() +
...
PlotHeat <- function(dataframe){
  heat_plot <-
    ...
    theme(plot.title = element_text(size = 10, face = "bold", hjust = 0.5),
          ...
          legend.title = element_text("Sum", size = 8, face = "bold",  
                                      hjust = 0, vjust = 0.5), 
    ...
    scale_fill_gradient2("Read No.", low="blue", mid="grey", high="red", #colors in the scale
