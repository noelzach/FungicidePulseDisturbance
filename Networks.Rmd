...
complexity.box <- ggplot(Network.complexity[Network.complexity$Crop == "Soy",], aes(x = DPF, y = complexity, fill = Fungicide)) + 
  ...
  xlab("")+
  stat_compare_means(method = "wilcox", aes(label = ..p.format..), hide.ns = T, label.y = 7.5, size = 6) +  
...
complexity.box.corn <- ggplot(Network.complexity[Network.complexity$Crop == "Corn",], aes(x = DPF, y = complexity, fill = Fungicide)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitterdodge(), shape = 21) +
  ylab("Linkage density") + 
  xlab("")+
...
nodes.box.corn <- ggplot(Network.complexity[Network.complexity$Crop == "Corn",], aes(x = DPF, y = n.nodes, fill = Fungicide)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitterdodge(), shape =21) +
  ylab("Nodes") + 
  xlab("")+
  stat_compare_means(method = "wilcox", aes(label = ..p.signif..), hide.ns = T, label.y = 350, size = 6) +  
  scale_fill_manual(values=c(cbbPalette[[1]], cbbPalette[[2]]), labels = c("Control", "Fungicide")) +
  theme_classic() +
  labs(color = "") + 
  ylim(c(0, 500)) +
...
ggpubr::ggarrange(complexity.box.corn, nodes.box.corn, nrow = 1, labels = c("(a)", "(b)"), common.legend = T)
```
