...
# filtering edges based on weight
#corn.graph2 <- delete.edges(meta.net.corn$igraph.object, which(E(meta.net.corn$igraph.object)$weight <= 0.05 & E(meta.net.corn$igraph.object)$weight >= -0.05))
soy.graph2 <- delete.edges(meta.net.soy$igraph.object, which(E(meta.net.soy$igraph.object)$weight <= 0.05 & E(meta.net.soy$igraph.object)$weight >= -0.05))
```
...
```{r}
network.scale.free.soy <- data.frame(degree(meta.net.soy$igraph.object)); colnames(network.scale.free.soy) <- "degree"
network.scale.free.soy$OTU <- row.names(network.scale.free.soy)
network.scale.free.soy <- left_join(mean.rel.abund.combined, network.scale.free.soy, by = "OTU")

ggplot(network.scale.free.soy, aes(x = degree, y = mean.relabund)) + 
  geom_point()

deg.dist <- as.data.frame(table(degree(meta.net.soy$igraph.object)))

deg.dist$Var1 <- as.numeric(deg.dist$Var1)

  ggplot(deg.dist, aes(x = log(Var1), y = log(Freq))) +
  geom_point() 

network.scale.free.corn <- data.frame(degree(corn.graph2)); colnames(network.scale.free.corn) <- "degree"
network.scale.free.corn$OTU <- row.names(network.scale.free.corn)
network.scale.free.corn <- left_join(mean.rel.abund.combined, network.scale.free.corn, by = "OTU")

ggplot(network.scale.free.corn, aes(x = degree, y = mean.relabund)) + 
  geom_point()

deg.dist <- as.data.frame(table(degree(corn.graph2)))

deg.dist$Var1 <- as.numeric(deg.dist$Var1)

  ggplot(deg.dist, aes(x = Var1, y = Freq)) +
  geom_point() +
...
complexity.box <- ggplot(Network.complexity[Network.complexity$Crop == "Soy",], aes(x = DPF, y = complexity, color = Fungicide)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitterdodge()) +
  ylab("Linkage density") + 
  xlab("")+
  stat_compare_means(method = "wilcox", aes(label = ..p.format..), hide.ns = T, label.y = 0.38, size = 6) +  
...
means.prok <- ggpubr::compare_means(N ~ Fungicide, data = positive.edges.prok, group.by = c("Class_from", "Crop", "Treatment_label", "DPF"), p.adjust.method = "fdr")
```
...
Network.edges %>%
  ...
  labs(color = "") + 
  coord_flip()+
  facet_wrap(Treatment_label~DPF, scales = "free_x", ncol = 6) +
  theme(strip.background = element_rect(color="white", fill="white", size=1.5, linetype="solid"),
   strip.text.x = element_text(size = 12, color = "black"),
